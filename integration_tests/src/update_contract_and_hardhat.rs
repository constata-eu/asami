...
fn main() {
    ...
    Command::new("truffle")
        .current_dir(&dir)
        .args(["compile", "test/MockDock.sol"])
        .output()
        .unwrap();
    ...
    println!("migrating");
    let migration = Command::new("truffle")
        .current_dir(&dir)
        .args(["migrate", "--network", "local"])
        .output()
        .unwrap();
    ...
    let output = Command::new("truffle")
        ...
        .args([
            "exec",
            "scripts/local_blockchain_state.js",
            "--network",
            "local",
        ])
        .output()
    ...
    serde_json::from_str::<Addresses>(json).unwrap_or_else(|_| {
        panic!("Local blockchain init script may have exited with an error.\n{out_str}")
    });
