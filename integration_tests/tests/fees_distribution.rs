...
app_test!{ distributes_fees_to_holders (a) 
  ...
  assert!(a.contract().distribute_fee_pool().send().await.is_err());
  ...
  // As the admin has 100% of the tokens issued, it gets the 100% of the feePool.
  assert_eq!(a.contract_doc_balance().await,     u("99990")); 
  ...
  // This is the pending DOC reward for Bob.
  // As the admin got 100% of the feePool earlier, it got its campaign money back, minus bob's reward.
  ...
  campaign = advertiser.create_x_campaign_valid_until(u("100000"), u("300"), 20).await;
