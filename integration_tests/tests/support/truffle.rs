...
impl Truffle {
    pub fn start() -> Self {
        ...
        Command::new("rm")
            .args(&["-rf", "../contract/ganache_data_copy"])
            .output()
            .expect("Could not remove previous log");
        Command::new("cp")
            .args(&["-r", "../contract/ganache_data", "../contract/ganache_data_copy"])
            .output()
            .expect("Could not kill previous server");
        ...
        let json = std::fs::read_to_string("../contract/ganache_data/addresses.json")
            .expect("cannot read ganache_data/addresses.json");
        let addresses: Addresses =
            serde_json::from_str(&json).expect("Wrong addresses.json file contents. Rebuild ganache state.");
