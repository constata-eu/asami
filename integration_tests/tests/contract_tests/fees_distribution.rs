...
async fn distributes_fees_to_holders() {
    TestHelper::run(|h| async move {
        ...
        let advertiser = h.advertiser_with_params(u("100000"), u("100000")).await;
        ...
        let mut bob = h.collaborator(200).await;
        bob.setup_trusted_admin("setting up trusted admin for bob").await;
        a.send_make_sub_account_collab_tx(
            "Bob collabs and gets tokens",
            "229400",
            &advertiser,
            u("1"),
            &bob,
        ...
        assert_eq!(a.asami_contract().total_supply().call().await.unwrap(), u("16000"));
        ...
        assert_eq!(
            a.asami_contract().get_fees_collected(wei("0")).call().await.unwrap(),
            u("10")
        );
        assert_eq!(
            a.asami_contract().get_issuance_for(u("1")).call().await.unwrap(),
            u("10000")
        );
        ...
        a.send_tx(
            ...
            "318768",
            a.asami_contract()
                .admin_claim_balances_free(vec![alice.address(), bob.address(), advertiser.address()]),
        )
        .await;
        a.send_tx(
            "Admin claims a second time",
            "151402",
            a.asami_contract().claim_balances(),
        )
        .await;
        ...
        assert_eq!(a.asami_contract().fee_pool().call().await.unwrap(), u("20"));
        ...
        assert_eq!(a.asami_contract().total_supply().call().await.unwrap(), u("240000"));
        ...
        a.send_tx(
            ...
            "213300",
            a.asami_contract().claim_fee_pool_share(vec![
        ...
        let mut susan = h.collaborator(200).await;
        susan.setup_trusted_admin("setting up trusted admin for susan").await;
        a.send_make_sub_account_collab_tx(
            "susan collabs and gets tokens",
            "216638",
            &advertiser,
            u("3"),
            &susan,
        ...
        assert_eq!(a.asami_contract().fee_pool().call().await.unwrap(), u("30"));
        assert_eq!(
            a.asami_contract().get_fee_pool_before_recent_changes().call().await.unwrap(),
            u("0")
        );
        ...
        a.send_revert_tx(
            "No holder gets paid as previous cycle had no collabs.",
            "cfp1",
            a.asami_contract().claim_fee_pool_share(holders.clone()),
        )
        .await;
        for h in &unknown_holders {
            assert_eq!(a.asami_balance_of(h).await, wei("100000000000005227"));
        ...
        a.evm_forward_to_next_cycle().await;
        a.send_tx(
            "everyone gets paid the 30 doc from last period",
            "6039459",
            a.asami_contract().claim_fee_pool_share(holders.clone()),
        )
        .await;
        ...
        assert_eq!(a.asami_contract().total_supply().call().await.unwrap(), u("270000"));
        ...
        assert_eq!(a.asami_contract().fee_pool().call().await.unwrap(), wei("72"));
    })
    .await;
}
