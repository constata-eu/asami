...
app_test! { has_an_adaptable_schedule_with_a_target_per_cycle (a)
  ...
  let do_collab = |context, amount|{
    a.send_make_collab_tx(context, "230000", &advertiser, campaign, &alice, amount)
  };
...
app_test! { rate_can_be_voted (a)
  ...
  a.send_make_collab_tx("Alice collabs and gets tokens", "230000", &advertiser, u("deadbeef"), &alice, budget).await;
  a.send_tx(
  ...
  a.send_tx("first apply changes rate", "55000", a.asami_contract().apply_voted_fee_rate()).await;
  ...
  a.send_revert_tx("re-apply in second cycle", "afr0", a.asami_contract().apply_voted_fee_rate()).await;
  assert_eq!(fee_rate(&a).await, u("20"));

  a.evm_forward_to_next_cycle().await;

  a.send_tx("third apply works on next cycle", "38000", a.asami_contract().apply_voted_fee_rate()).await;
  ...
  a.send_tx("Will remove votes when sending tokens", "111000",
