...
async fn migrating_tokens_from_asami_v1() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.collaborator(200).await;
        let alice = h.collaborator(200).await;
        let bob = h.collaborator(200).await;
        let stranger = h.collaborator(200).await;
        ...
        alice
            .legacy_contract()
            .transfer(stranger.address(), milli("50"))
            .send()
            .await
            .unwrap()
            .await
        ...
        assert_eq!(
            ...
            vec![
                a.admin_treasury_address().await,
                advertiser.address(),
                alice.address(),
                stranger.address()
            ]
        ...
        a.send_tx(
            ...
            "478045",
            a.asami_contract().migrate_tokens_from_old_contract(
                a.legacy_contract().address(),
        ...
        let advertiser_account = a
            .asami_contract()
            .accounts(advertiser.address())
            .call()
            .await
            .unwrap_or_else(|_| panic!("could not fetch advertiser's account"));
        assert_eq!(
            advertiser_account,
            (
                a.client_admin_address(),
        ...
        let alice_account = a
            ...
            .call()
            .await
            .unwrap_or_else(|_| panic!("could not fetch alice's account"));
...
async fn assert_legacy_account(
    ...
    let account = a
        ...
        .call()
        .await
        .unwrap_or_else(|_| panic!("could not fetch account {account_id}"));
