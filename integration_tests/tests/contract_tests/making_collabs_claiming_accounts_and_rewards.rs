...
async fn full_contract_workflow_from_collabs_to_balance_claims() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        ...
        let collabs_params = vec![MakeSubAccountCollabsParam {
            ...
            collabs: (0..20)
                .map(|i| MakeSubAccountCollabsParamItem {
                    sub_account_id: wei((i + 1).to_string()),
                    doc_reward: u("20"),
                })
                .collect(),
        }];
        ...
        .await;
        a.assert_sub_account_balances_of("alice subaccount after promote", alice.account_id(), u("0"), u("0"))
            .await;
        a.assert_sub_account_balances_of("bob subaccount after promote", bob.account_id(), u("0"), u("0"))
            .await;
        ...
        a.send_tx(
            ...
            "387103",
            a.asami_contract()
                .gasless_claim_balances(admin_doc_fee, u("1"), vec![alice.address(), bob.address()])
                .value(u("2")),
        )
        .await;

        a.assert_balances_of(
            "alice after gasless claim",
            alice.address(),
            milli("1010"),
            u("0"),
            u("33"),
        ...
        a.assert_balances_of(
            "carl after regular claim",
            carl.address(),
            wei("9878661999150634"),
            u("0"),
            u("36"),
            u("0"),
            u("4800"),
        ...
        a.assert_admin_balances(
            "Before its own claim.",
            u("6"),
            u("419990000"),
            u("99160000000"), // 6 unclaimed doc from gasless
            u("128000"),
            u("0"),
            u("0"),
        )
...
#[tokio::test]
#[serial_test::file_serial]
async fn rejects_collabs_when_too_large_or_campaign_is_finished() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        ...
        let make_collabs_call = |doc_reward: &str| {
            a.asami_contract().admin_make_sub_account_collabs(vec![MakeSubAccountCollabsParam {
                advertiser_addr: advertiser.address(),
                briefing_hash,
                collabs: vec![MakeSubAccountCollabsParamItem {
                    sub_account_id: wei("1"),
                    doc_reward: u(doc_reward),
                }],
            }])
        };
...
#[tokio::test]
#[serial_test::file_serial]
async fn gasless_claim_common_error_cases() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        advertiser.pay_campaign("global campaign for test", u("2000"), u("deadbeef"), 2).await;

        let mut alice = h.collaborator(200).await;
        ...
        a.send_revert_tx(
            ...
            "gcb0",
            a.asami_contract()
                .gasless_claim_balances(u("1"), wei("1000000"), vec![alice.address()])
                .value(wei("1000000")),
        )
        .await;
        ...
        a.send_revert_tx(
            ...
            "gcb4",
            a.asami_contract()
                .gasless_claim_balances(wei("10000000"), u("1"), vec![alice.address()])
                .value(u("1")),
        )
        .await;
        ...
        a.assert_balances_of(
            ...
            alice.address(),
            wei("1009928884999502195"),
            u("0"),
            wei("899999999990000000"),
            u("0"),
            u("120"),
        )
        .await;
...
#[tokio::test]
#[serial_test::file_serial]
async fn user_can_manage_a_gasless_amount_approval() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        advertiser.pay_campaign("global campaign for test", u("2000"), u("deadbeef"), 2).await;
        ...
        a.send_tx(
            ...
            "102000",
            a.asami_contract().promote_sub_accounts(vec![PromoteSubAccountsParam {
                id: alice.account_id(),
                addr: alice.address(),
            }]),
        )
        ...
        let (trusted_admin, max_gasless_doc_to_spend, min_gasless_rbtc_to_receive, _, _, _, _) =
            a.asami_contract().accounts(alice.address()).call().await.unwrap();
        ...
        a.send_revert_tx(
            "The admin cannot charge more than the default gasless approval",
            "gcb4",
            expensive_gasless_claim.clone(),
        )
        .await;
        ...
        a.send_tx(
            "The admin can now do an expensive gasless claim.",
            "257650",
            expensive_gasless_claim.clone(),
        )
        .await;
        ...
        a.send_revert_tx(
            "The admin cannot do gasless claims",
            "gcb2",
            expensive_gasless_claim.clone(),
        )
        .await;
...
#[tokio::test]
#[serial_test::file_serial]
async fn simultaneous_gasless_and_regular_claims_fail() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        advertiser.pay_campaign("global campaign for test", u("2000"), u("deadbeef"), 2).await;
...
#[tokio::test]
#[serial_test::file_serial]
async fn admin_can_maintain_sub_accounts_for_users() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        let briefing_hash = u("deadbeef");
        ...
        let alice_sub_account_collab =
            a.asami_contract().admin_make_sub_account_collabs(vec![MakeSubAccountCollabsParam {
                advertiser_addr: advertiser.address(),
                briefing_hash,
                collabs: vec![MakeSubAccountCollabsParamItem {
                    sub_account_id: alice.account_id(),
                    doc_reward: u("10"),
                }],
            }]);

        let promote_alice = a.asami_contract().promote_sub_accounts(vec![PromoteSubAccountsParam {
            id: alice.account_id(),
            addr: alice.address(),
        }]);

        a.send_tx(
            "Admin keeps a sub account for alice",
            "391798",
            alice_sub_account_collab.clone(),
        )
        .await;
        ...
        a.send_tx(
            "Admin registers more sub account collabs for alice",
            "391798",
            alice_sub_account_collab,
        )
        .await;
...
#[tokio::test]
#[serial_test::file_serial]
async fn can_change_admin_for_registering_collabs() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        let briefing_hash = u("deadbeef");
        ...
        let alice = h.collaborator(200).await;
        let other_admin = h.user().await;

        let sub_account_collab =
            other_admin.asami_contract().admin_make_sub_account_collabs(vec![MakeSubAccountCollabsParam {
                advertiser_addr: advertiser.address(),
                briefing_hash,
                collabs: vec![MakeSubAccountCollabsParamItem {
                    sub_account_id: alice.account_id(),
        ...
        let regular_collab = other_admin.asami_contract().admin_make_collabs(vec![MakeCollabsParam {
            advertiser_addr: advertiser.address(),
            briefing_hash,
            collabs: vec![MakeCollabsParamItem {
                account_addr: alice.address(),
                doc_reward: u("10"),
            }],
        }]);

        a.send_revert_tx(
            "Sub account collab from new admin fails",
            "amc1",
            sub_account_collab.clone(),
        )
        .await;
