...
async fn full_campaign_workflow_until_reimbursed() {
    TestHelper::run(|h| async move {
        ...
        a.send_make_collab_tx(
            "a large collab drains all the funds",
            "226000",
            &advertiser,
            campaign,
            &alice,
        ...
        a.send_tx(
            ...
            "53200",
            a.submit_report_contract_call(account, campaign, u("1")),
        )
        .await;
        ...
        a.send_revert_tx(
            "no collab allowed",
            "amc2",
            a.asami_contract().admin_make_collabs(vec![MakeCollabsParam {
        ...
        a.send_tx(
            "campaign is topped up again",
            "78582",
            advertiser.top_up_campaign_contract_call(account, campaign, u("100")),
        )
        .await;

        a.send_make_collab_tx("a new smaller collab", "132000", &advertiser, campaign, &alice, u("20"))
            .await;

        assert_eq!(advertiser.doc_balance().await, u("1997850"));

        a.evm_forward_to_next_cycle().await;
        a.send_tx(
            "Campaign is reimbursed again",
            "47200",
            advertiser.reimburse_campaign_contract_call(account, campaign),
        )
        .await;

        assert_eq!(advertiser.doc_balance().await, u("1997930"));
...
#[tokio::test]
#[serial_test::file_serial]
async fn campaign_creation_validations() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let campaign = u("deadbeef");
        ...
        a.send_revert_tx(
            "needs budget > 0",
            "mc0",
            advertiser.pay_campaign_contract_call(u("0"), campaign, 1),
        )
        .await;
        ...
        a.send_revert_tx(
            ...
            "mc1",
            advertiser.pay_campaign_contract_call(u("1000"), campaign, -1),
        )
        .await;
...
#[tokio::test]
#[serial_test::file_serial]
async fn campaign_extension_validations() {
    TestHelper::run(|h| async move {
        let a = h.a();
        ...
        let other = h.advertiser_with_params(u("10000"), u("10000")).await;

        a.send_revert_tx(
            "can't extend unknown campaign",
            "ec1",
        ...
        a.send_revert_tx(
            "can't make campaign end sooner",
            "ec2",
...
#[tokio::test]
#[serial_test::file_serial]
async fn campaign_top_up_tests() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        let campaign = u("deadbeef");
        ...
        a.send_revert_tx(
            "can't top-up a non existing campaign",
            "tc2",
        ...
        a.send_tx(
            "can top-up someone else's campaign",
            "61500",
...
async fn campaign_submit_report_tests() {
    TestHelper::run(|h| async move {
        ...
        let stranger = h.user().await;

        a.send_revert_tx(
            "can't submit report before campaign finishes",
            "sr3",
        ...
        a.send_revert_tx(
            "can't submit report for unknown user",
            "sr1",
        ...
        a.send_revert_tx(
            "can't submit report for unknown campaign",
            "sr2",
        ...
        a.send_revert_tx(
            "only admin can submit report",
            "sr1",
            advertiser.asami_contract().submit_reports(vec![SubmitReportsParam {
                account,
                briefing_hash: campaign,
                report_hash: u("1000"),
            }]),
        ...
        a.send_revert_tx(
            "can't submit report if already present",
            "sr4",
...
#[tokio::test]
#[serial_test::file_serial]
async fn campaign_reimbursement_tests() {
    TestHelper::run(|h| async move {
        let a = h.a();
        let advertiser = h.advertiser_with_params(u("10000"), u("10000")).await;
        let campaign = u("deadbeef");
        ...
        a.send_revert_tx(
            "cannot reimburse before campaign finishes",
            "rc1",
        ...
        a.send_revert_tx(
            "cannot reimburse for unknown account",
            "rc0",
