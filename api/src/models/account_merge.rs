...
impl AccountMerge {
    ...
    pub async fn accept(&self, source: Account) -> AsamiResult<()> {
        ...
        tx.db.execute(sqlx::query!("UPDATE handles SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE sessions SET logged_out_at = now() WHERE account_id = $1", source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE account_users SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE campaigns SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE campaigns SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE collabs SET member_id = $1 WHERE member_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE collabs SET advertiser_id = $1 WHERE advertiser_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE on_chain_job_accounts SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE campaign_preferences SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE community_members SET account_id = $1 WHERE account_id = $2", self.destination_id(), source.id())).await?;
        tx.db.execute(sqlx::query!("UPDATE community_members SET member_id = $1 WHERE member_id = $2", self.destination_id(), source.id())).await?;

        tx.account().find(source.id()).await?
            .update()
            .force_hydrate(true)
            ...
            .save()
            .await?;
        tx.account().find(self.destination_id()).await?
            .update()
            .force_hydrate(true)
            .save()
            .await?;
        
        tx.account_merge().find(self.id()).await?.update()
