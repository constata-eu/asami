...
impl BackerDisbursementHub {
    pub async fn create_from_events(
        &self,
        tx_hash: H256,
        events: &[BuilderRewardsClaimedFilter],
    ) -> anyhow::Result<Option<BackerDisbursement>> {
        ...
        let raw_issuance = self
            .state
            .on_chain
            .asami_contract
            .get_issuance_for(u("1"))
            .call()
            .await
            .context("getting asami issuance rate")?;
...
impl InsertBackerDisbursementHub {
    pub async fn schedule_and_save(self) -> AsamiResult<BackerDisbursement> {
        ...
        saved
            ...
            .db
            .execute(sqlx::query!(
                "UPDATE backer_stakes SET disbursement_id = $1 WHERE disbursement_id IS NULL",
                *saved.id()
            ))
            .await?;
...
impl BackerHub {
    pub async fn get_or_create(&self, addr: Address) -> sqlx::Result<Backer> {
        ...
        self.insert(InsertBacker {
            address: addr.encode_hex(),
        })
        .save()
        .await
    }
...
impl BackerPayoutHub {
    pub async fn pay_all(&self) -> AsamiResult<()> {
        ...
        if unpaid.is_empty() {
            self.state.info("backer_payouts", "pay_all", "no_unpaid_backer_payouts").await;
            return Ok(());
        }
        ...
        if needed_balance > current_balance {
            return Err(Error::precondition(&format!(
                "not_enough_asami_to_pay_backers, needed: {needed_balance}, current: {current_balance}"
            )));
        }
